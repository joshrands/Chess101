...
| OPEN-10 | `detect_mismatch` only checks for missing pieces, not extra pieces. It iterates each team's piece list and verifies the reed switch reads OCCUPIED, but never checks that cells with no software piece read EMPTY. On the real Pi this is mostly harmless (players physically manage pieces), but in the HIL stale reed switch state from earlier phases (lobby, color_picker, war_games) persists and interferes with gameplay — `detect_lift_off` and `detect_landing` see phantom pieces the user never placed. Fix: extend detect_mismatch to also flag cells that are OCCUPIED but have no corresponding piece in `self.grid`. Suspected: `game/board.py` — `Board.detect_mismatch`. |
