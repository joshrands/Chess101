...
# Relay fuzzer — randomised delivery invariant testing (ordering, completeness, reconnect):
.venv/bin/python harness/fuzz_relay.py --iterations 100
.venv/bin/python harness/fuzz_relay.py --iterations 100 --seed 7
...
```bash
# Preferred — runs everything (Python + JS + Swift) via Bazel:
...
# Relay fuzzer — delivery invariants (ordering, completeness, reconnect, spectator):
bazel run //harness:fuzz_relay -- --iterations 100
bazel run //harness:fuzz_relay -- --iterations 100 --seed 7

# Fuzzer options (all three lockstep fuzzers accept these):
...
# fuzz_relay options:
#   --iterations N    Total fuzzing iterations (default 100)
...
## Chess101iOS (Swift App)

Native iOS client for Chess101. Connects to the relay server (`wss://relay.chess101.net`) to play internet games, scan ChessMatrix room codes via the camera, and render the board in either 2D or 3D.
...
# Build and run via Swift Package Manager (unit tests only, no UI):
cd Chess101iOS && swift test && cd ..

# Build and run on a simulator or device:
# Open Chess101iOS/Chess101iOS.xcodeproj in Xcode, then Cmd+R.
```

`project.yml` is the source of truth for build settings. Never edit the `.xcodeproj` directly — regenerate it with `xcodegen generate` after any changes to `project.yml`.

### Package Structure

```
Chess101iOS/
├── Package.swift                         # Swift package manifest (Chess101Engine + Chess101iOS targets)
├── project.yml                           # XcodeGen spec: targets, plist keys, build settings
│
├── Sources/
│   ├── Chess101Engine/                   # Pure Swift chess logic (no UIKit / SwiftUI)
│   │   └── ChessEngine.swift            # Board state, legal move generation, alpha-beta AI
│   │
│   ├── SwiftBridge/                      # Python↔Swift fuzz bridge (used by fuzz_lockstep_swift.py)
│   │   └── SwiftBridge.swift
│   │
│   └── Chess101iOS/                      # SwiftUI app target
│       ├── Chess101App.swift             # @main entry — splash gate before ContentView loads
│       ├── Assets.xcassets/              # App icon, accent color, LaunchBackground color asset
│       ├── Models/                       # OBJ files: pawn/rook/bishop/knight/queen/king (generated by tools/stl_to_obj.py)
│       │
│       ├── AI/
│       │   └── AIPlayer.swift           # Async AI move selection (wraps ChessEngine alpha-beta)
│       │
│       ├── Game/
│       │   └── GameSession.swift        # ObservableObject: board state, turn management, relay integration
│       │
│       ├── Network/
│       │   └── RelayClient.swift        # URLSessionWebSocketTask relay client (create/join/spectate/reconnect)
│       │
│       └── UI/
│           ├── ContentView.swift        # Root view: SplashView definition + main game layout
│           ├── LobbyView.swift          # Host/join/spectate lobby (ChessMatrix scanner sheet)
│           ├── Board2DView.swift        # SwiftUI 2D board (LED-style squares, tap to move)
│           ├── Board3DView.swift        # SceneKit 3D board (OBJ pieces, LED emission, arc animations)
│           └── ScannerView.swift        # AVFoundation camera view for ChessMatrix barcode scanning
│
└── Tests/
    └── Chess101IOSTests/
        └── Chess101IOSTests.swift       # Unit tests for ChessEngine (run via `swift test`)
```

### Architecture

**App launch**: `Chess101App` (in `Chess101App.swift`) owns a `@State var splashDone` flag. On the very first SwiftUI frame it renders `SplashView`; after the animation completes it cross-fades to `ContentView`. This ensures the splash is visible during the full OS binary-loading wait (up to 15 s on cold boot).

**SplashView**: 8×8 chessboard tiles assemble with staggered spring animations, "CHESS 101" fades in at 0.65 s, `onFinish` callback fires at 1.8 s.

**GameSession**: central `ObservableObject` that owns board state (`board: [[Piece?]]`), turn tracking, move history, and relay integration. Views observe it via `@ObservedObject`.

**RelayClient**: `URLSessionWebSocketTask`-based WebSocket client. Uses a `waiters: [Waiter]` list of `CheckedContinuation`s for race-free one-shot handshake responses. Handshake messages are consumed by the first matching waiter; game messages are forwarded to `messageStream: AsyncStream`.

Key invariant: `connectAndWait()` suspends until `urlSession(_:webSocketTask:didOpenWithProtocol:)` fires before any message is sent. Never call `send()` before `connectAndWait()` returns.

**Board3DView**: SceneKit view with a `CADisplayLink` for 60 fps LED emission updates.
- All squares use `.constant` lighting model (pure emitters — no scene lighting needed).
- Piece OBJ templates loaded once at init; each board cell gets a `.clone()`.
- Move animations: `SCNAction.customAction` with ease-out + `sin(t·π)` Y arc.
- AI thinking: traveling wave `sin(t·3.2 + (r+c)·0.45)` pulsed across all squares.
- Tap handling: walks SCNNode parent hierarchy for `"sq_r_c"` / `"piece_r_c"` node names.

**ScannerView**: `AVCaptureSession` with `AVCaptureVideoDataOutput`. `startRunning()` must be called on a background thread (`DispatchQueue.global(qos: .userInitiated)`). Decoded frames are passed to the ChessMatrix Swift decoder.

### Regenerating OBJ piece models

The 3D piece models are generated from the STL files in the repo:

```bash
# Convert all STL → OBJ (centered at XZ origin, smooth normals):
.venv/bin/python tools/stl_to_obj.py
# Outputs: Chess101iOS/Sources/Chess101iOS/Models/{pawn,rook,knight,bishop,queen,king}.obj
```

Re-run `xcodegen generate` after adding or removing OBJ files so Xcode picks up the new resources.

### Info.plist keys (set via project.yml)

| Key | Value |
|---|---|
| `NSCameraUsageDescription` | Camera used to scan ChessMatrix room codes |
| `UILaunchScreen_BackgroundColorName` | `LaunchBackground` (dark navy matching app bg) |
